    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Находит наименьшую стоимость при ограничениях на качество белка.
            Все поля необязательны — оставьте пустыми для чистой минимизации стоимости.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-5 py-1">
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <Label className="text-xs text-muted-foreground">БЦ ≥ (%, необяз.)</Label>
                <Input
              ...
              <div className="space-y-1">
                <Label className="text-xs text-muted-foreground">КРАС ≤ (%, необяз.)</Label>
                <Input
          ...
          {eligibleRows.length > 0 && (
            <>
              <Separator />
              <div>
                <p className="mb-2 text-sm font-medium">Диапазоны X<sub>i</sub>, г</p>
                ...
                <div className="space-y-2">
                  <div className="grid grid-cols-[1fr_90px_90px] gap-2 px-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">
                    <span>Сырьё</span><span>Мин, г</span><span>Макс, г</span>
                  </div>
                  {eligibleRows.map((row) => {
                    const bound = bounds.find((b) => b.key === row.key)
                    return (
                      <div key={row.key} className="grid grid-cols-[1fr_90px_90px] items-center gap-2">
                        <span className="truncate text-sm">{productName(row.product_id)}</span>
                        <Input
                          type="number" min="0" max="100" step="0.1"
                          placeholder="0"
                          value={bound?.min ?? ''}
                          onChange={(e) => patchBound(row.key, { min: e.target.value })}
                        />
                        <Input
                          type="number" min="0" max="100" step="0.1"
                          placeholder="100"
                          value={bound?.max ?? ''}
                          onChange={(e) => patchBound(row.key, { max: e.target.value })}
                        />
                      </div>
                    )
                  })}
