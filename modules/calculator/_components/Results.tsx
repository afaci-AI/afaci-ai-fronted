  ...
  return (
    <Card className={cn('border', s.card)}>
      <CardHeader>
        ...
        <CardDescription>
          Автоматическая оценка качества белка по показателям расчёта
        </CardDescription>
      </CardHeader>
      <CardContent>
...
export function Results({ result }: { result: ComputeResult }) {
  ...
  const utilData = sortedAmino
    ...
    .map((a) => ({
      ...
      utility: a.utility,
      is_min: a.is_min,
      color: a.is_min ? COL.min : COL.score,
    }))
  ...
  return (
    <div id="calc-results" className="space-y-6">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          label="Энергоценность"
          icon={Zap}
          value={
            <>
              {nf(result.energy_kcal, 0)}{' '}
        ...
        />
        <StatCard
          label="Лимитирующая НАК (Cₘᵢₙ)"
          icon={AlertTriangle}
          value={
            result.c_min
              ? `${result.c_min.name} · ${nf(result.c_min.score, 1)}%`
              : '—'
          }
          ...
          <CardTitle className="flex items-center gap-2 text-lg">
            ...
            <FormulaHint>
              ...
              <p className="text-muted-foreground mt-1">
                Доля компонента — сумма вкладов каждого ингредиента (его
                содержание × масса), делённая на 100.
              </p>
            </FormulaHint>
          </CardTitle>
          <CardDescription>
            Белки, жиры, углеводы и пищевые волокна, г на 100 г продукта
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <CardTitle className="flex items-center gap-2 text-lg">
            ...
            <FormulaHint>
              ...
              <p className="font-medium mt-1">
                Скор C = Mⱼ ÷ эталон × 100&nbsp;%
              </p>
              <p className="text-muted-foreground mt-1">
                Эталон — «{result.reference.name}». Минимальный скор =
                лимитирующая аминокислота.
              </p>
            </FormulaHint>
          </CardTitle>
          <CardDescription>
            Скор каждой незаменимой аминокислоты относительно эталона «
            {result.reference.name}». Пунктир — уровень эталона (100%).
          </CardDescription>
        </CardHeader>
              ...
              >
                ...
                <Bar dataKey="score" radius={[4, 4, 0, 0]}>
                  ...
                  <LabelList
                    ...
                    position="top"
                    fontSize={11}
                    formatter={(v: number) => nf(Number(v), 0)}
                  />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ...
          <Table>
            ...
            <TableBody>
              {sortedAmino.map((a) => (
                ...
                >
                  <TableCell
                    className={cn('font-medium', a.is_min && 'font-bold')}
                  >
                    {a.name}
                  </TableCell>
                  <TableCell className="text-right tabular-nums">
            ...
            <div className="border-destructive/40 bg-destructive/5 flex items-center gap-3 rounded-lg border p-4">
              ...
              <div>
                <div className="font-semibold">
                  Лимитирующая аминокислота (Cₘᵢₙ): {result.c_min.name} —{' '}
                  {nf(result.c_min.score, 1)}%
                </div>
                <div className="text-muted-foreground text-sm">
                  Именно она ограничивает биологическую ценность белка. Всего
                  скор ниже 100% у {result.limiting_count} НАК:{' '}
                  {result.limiting.join(', ')}.
                </div>
              </div>
            </div>
          )}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Сбалансированность и реализуемость аминокислотного состава
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            />
            <QualityCard
              label="Биологическая ценность (БЦ)"
              value={`${nf(q.bc, 1)} %`}
            ...
            />
            <QualityCard
              label="Сопоставимая избыточность G"
              value={`${nf(q.G, 2)}`}
                  ...
                  >
                    ...
                    >
                      <LabelList
                        ...
                        fontSize={13}
                        fill="#fff"
                        formatter={(v: number) => `БЦ ${nf(Number(v), 1)}%`}
                      />
                    </Bar>
                    <Bar
                      dataKey="kras"
                    ...
                    >
                      <LabelList
                        ...
                        fontSize={13}
                        fill="#fff"
                        formatter={(v: number) => `КРАС ${nf(Number(v), 1)}%`}
                      />
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
