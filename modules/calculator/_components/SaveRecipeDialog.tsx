...
export function SaveRecipeDialog({
  ...
  const persist = async (asNew: boolean) => {
    ...
    if (groupChoice === NEW_GROUP && !newGroupName.trim()) { toast.error('Введите название новой группы'); return }
    setSaving(true)
    try {
      if (isEdit && !asNew) {
        const body: any = {
        ...
        toast.success(isDraft ? `Черновик «${name.trim()}» сохранён` : `Рецептура «${name.trim()}» сохранена`)
      }
      setOpen(false)
      setNewGroupName('')
    } catch (e: any) {
