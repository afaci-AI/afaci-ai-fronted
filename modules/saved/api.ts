...
export const savedApi = {
  ...
  createGroup: (data: { name: string }) =>
    fetchApi<SavedGroup>('/saved/groups', { method: 'POST', body: JSON.stringify(data) }),
  updateGroup: (id: string, data: { name: string }) =>
    fetchApi<SavedGroup>(`/saved/groups/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  deleteGroup: (id: string) =>
    fetchApi<void>(`/saved/groups/${id}`, { method: 'DELETE' }),

  recipes: (groupId?: string) =>
    fetchApi<SavedRecipe[]>(`/saved/recipes${groupId ? `?group_id=${groupId}` : ''}`),
  recipe: (id: string) => fetchApi<SavedRecipeDetail>(`/saved/recipes/${id}`),
  createRecipe: (body: SaveRecipeBody) =>
    fetchApi<SavedRecipeDetail>('/saved/recipes', { method: 'POST', body: JSON.stringify(body) }),
  updateRecipe: (id: string, body: UpdateRecipeBody) =>
    fetchApi<SavedRecipeDetail>(`/saved/recipes/${id}`, { method: 'PATCH', body: JSON.stringify(body) }),
  ...
  ranking: (body: { recipe_ids: string[]; weights?: { bc: number; kras: number; v: number; g: number } }) =>
    fetchApi<RankingResult>('/saved/ranking', { method: 'POST', body: JSON.stringify(body) }),
}
