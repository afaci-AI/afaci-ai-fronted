...
export const productsApi = {
  ...
  byCategory: (id: string) =>
    fetchApi<Product[]>(`/products/by-category/${id}`),
  auto: (data: {
    name: string
    ...
    fetchApi<ProductWithRelations>('/products/auto', {
      method: 'POST',
      body: JSON.stringify(data),
    }),
}
...
export const nutrientsApi = {
  ...
  create: (data: NutrientCreate) =>
    fetchApi<Nutrient>('/nutrients', {
      method: 'POST',
      body: JSON.stringify(data),
    }),
  update: (id: string, data: NutrientUpdate) =>
    fetchApi<Nutrient>(`/nutrients/${id}`, {
      method: 'PATCH',
      body: JSON.stringify(data),
    }),
  delete: (id: string) =>
    fetchApi<void>(`/nutrients/${id}`, { method: 'DELETE' }),
  byProduct: (id: string) =>
    fetchApi<NutrientWithRelations[]>(`/nutrients/product/${id}`),
}
...
export const tableApi = {
  products: (limit = 1000) =>
    fetchApi<CalcProduct[]>(`/table/products?limit=${limit}`),
}
