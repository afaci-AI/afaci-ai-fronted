...
const statusBadgeVariants: Record<UserStatus, 'default' | 'secondary' | 'outline' | 'destructive'> = {
  active: 'outline',
  unlimited: 'outline',
  blocked: 'secondary',
  expired: 'destructive',
...
export default function UsersPage() {
  ...
  const validate = (): boolean => {
    ...
    } else if (!formData.unlimited && new Date(formData.accessExpiresAt).getTime() <= Date.now()) {
      newErrors.accessExpiresAt = 'Дата окончания доступа не может быть в прошлом'
    }
    setErrors(newErrors)
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const accessExpiresAtIso = formData.unlimited || !formData.accessExpiresAt
        ? null
      ...
      if (selectedUser) {
        const updated = await usersApi.update(selectedUser.id, {
          ...
          access_expires_at: formData.unlimited ? undefined : accessExpiresAtIso,
        })
  ...
  const handleToggleStatus = async (user: AdminUser) => {
    try {
      if (user.isActive) {
        ...
        setUsers((prev) => prev.map((u) => (u.id === user.id ? { ...u, isActive: false, status: 'blocked' } : u)))
    ...
    <>
      ...
      <main className="flex-1 overflow-auto p-4 md:p-6">
        <Card>
          <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <CardDescription>Управление пользователями, ролями и сроком доступа</CardDescription>
            </div>
                  ...
                  {isLoadingUsers ? (
                    ...
                    filteredUsers.map((user) => (
                      <TableRow key={user.id} className={!user.isActive ? 'opacity-60' : ''}>
                        ...
                        <TableCell className={isExpiringSoon(user.accessExpiresAt) ? 'font-medium text-amber-600' : 'text-muted-foreground'}>
                          {user.accessExpiresAt
                            ? new Date(user.accessExpiresAt).toLocaleDateString('ru-RU', {
        ...
        <Sheet open={drawerOpen} onOpenChange={setDrawerOpen}>
          <SheetContent>
            <SheetHeader>
              <SheetTitle>{selectedUser ? 'Редактировать пользователя' : 'Создать пользователя'}</SheetTitle>
              <SheetDescription>
                {selectedUser ? 'Измените данные пользователя' : 'Заполните данные нового пользователя'}
              </SheetDescription>
            </SheetHeader>
            ...
            <form onSubmit={handleSave} className="mt-6 space-y-4 px-4">
              <FieldGroup>
                ...
                <Field>
                  ...
                  <Select value={formData.role} onValueChange={(v) => handleChange('role', v as UserRole)}>
                    <SelectTrigger>
                ...
                <Field>
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) => handleChange('unlimited', checked)}
                    />
                  </div>
                  {!formData.unlimited && (
                    <>
                      ...
                      />
                      {errors.accessExpiresAt && <FieldMessage variant="error">{errors.accessExpiresAt}</FieldMessage>}
                    </>
                  )}
                </Field>
              </FieldGroup>
