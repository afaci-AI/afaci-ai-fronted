...
export default function ProductDetailPage({
  ...
  const handleOpenDrawer = (nutrient?: Nutrient) => {
    if (nutrient) {
      ...
      setFormData({
        nutrient_name_id: nutrient.nutrient_name_id,
        unit_id: nutrient.unit_id,
        quantity: String(nutrient.quantity),
      })
    } else {
            ...
            <CardContent>
              <div className="grid gap-4 sm:grid-cols-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs text-muted-foreground">Нутриентов</p>
                    <p className="font-medium">{nutrients.length}</p>
                  </div>
                </div>
              </div>
            </CardContent>
          ...
          <Card>
            ...
            <CardContent>
              {nutrients.length === 0 ? (
                ...
                (() => {
                  const grouped = nutrientTypes
                    .map((type) => ({
                      type,
                      items: nutrients.filter((n) => {
                        const nm = nutrientNames.find(
                          (x) => x.id === n.nutrient_name_id,
                        )
        ...
        <Sheet open={drawerOpen} onOpenChange={setDrawerOpen}>
          <SheetContent>
            <SheetHeader>
              <SheetTitle>
                {selectedNutrient
                  ? 'Редактировать нутриент'
                  : 'Добавить нутриент'}
              </SheetTitle>
              <SheetDescription>
                Укажите нутриент и его количество
              </SheetDescription>
            </SheetHeader>
            <form onSubmit={handleSaveNutrient} className="mt-6 space-y-4">
              <FieldGroup>
                <Field>
                  ...
                  <Select
                    value={formData.nutrient_name_id}
                    onValueChange={(v) => handleChange('nutrient_name_id', v)}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Выберите нутриент" />
                    </SelectTrigger>
                    <SelectContent>
                      {nutrientNames.map((n) => (
              ...
              <SheetFooter className="mt-6">
                ...
                <Button type="submit" disabled={isSubmitting}>
                  ...
                  {selectedNutrient ? 'Сохранить' : 'Добавить'}
                </Button>
              </SheetFooter>
            </form>
          </SheetContent>
